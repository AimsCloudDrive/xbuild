...
import path$1, { sep } from "node:path";
...
//#endregion
//#region node_modules/rolldown/dist/shared/binding-ESkOU1xr.mjs
...
const INVALID_LOG_POSITION = "INVALID_LOG_POSITION", PLUGIN_ERROR = "PLUGIN_ERROR", INPUT_HOOK_IN_OUTPUT_PLUGIN = "INPUT_HOOK_IN_OUTPUT_PLUGIN", CYCLE_LOADING = "CYCLE_LOADING", PARSE_ERROR = "PARSE_ERROR";
...
function noop(..._args) {}
//#endregion
//#region node_modules/rolldown/dist/shared/error-BrnLyQ-g.mjs
...
//#endregion
//#region node_modules/rolldown/dist/shared/parse-_9Vwp6y3.mjs
...
//#endregion
//#region node_modules/@rolldown/pluginutils/dist/filter/composable-filters.js
var And = class {
	kind;
	args;
	constructor(...args) {
...
const nativeAppend = import_binding$3.BindingMagicString.prototype.append;
const nativePrepend = import_binding$3.BindingMagicString.prototype.prepend;
const nativeAppendLeft = import_binding$3.BindingMagicString.prototype.appendLeft;
const nativeAppendRight = import_binding$3.BindingMagicString.prototype.appendRight;
const nativePrependLeft = import_binding$3.BindingMagicString.prototype.prependLeft;
const nativePrependRight = import_binding$3.BindingMagicString.prototype.prependRight;
const nativeOverwrite = import_binding$3.BindingMagicString.prototype.overwrite;
const nativeUpdate = import_binding$3.BindingMagicString.prototype.update;
...
const nativeReplace = import_binding$3.BindingMagicString.prototype.replace;
const nativeReplaceAll = import_binding$3.BindingMagicString.prototype.replaceAll;
...
* @experimental
*/
const RolldownMagicString = import_binding$3.BindingMagicString;
...
var prompt_U5ajztzG_exports = /* @__PURE__ */ __exportAll({ prompt: () => prompt });
...
var init_prompt_U5ajztzG = __esmMin((() => {
...
//#endregion
//#region node_modules/rolldown/dist/shared/rolldown-build-D_ShytiL.mjs
...
var FancyReporter = class extends BasicReporter {
	formatStack(stack, message, opts) {
		const indent = "  ".repeat((opts?.errorLevel || 0) + 1);
		return `
${indent}` + parseStack(stack, message).map((line) => "  " + line.replace(/^at +/, (m) => colors$1.gray(m)).replace(/\((.+)\)/, (_, m) => `(${colors$1.cyan(m)})`)).join(`
${indent}`);
	}
...
function characterFormat(str) {
	return str.replace(/`([^`]+)`/gm, (_, m) => colors$1.cyan(m)).replace(/\s+_([^_]+)_\s+/gm, (_, m) => ` ${colors$1.underline(m)} `);
}
...
function createConsola(options = {}) {
	...
	return createConsola$1({
		...
		prompt: (...args) => Promise.resolve().then(() => (init_prompt_U5ajztzG(), prompt_U5ajztzG_exports)).then((m) => m.prompt(...args)),
...
* Console logger
*/
const logger$1 = process.env.ROLLDOWN_TEST ? createTestingLogger() : createConsola({ formatOptions: { date: false } });
...
function bindingifyOutputOptions(outputOptions) {
	...
	if (legalComments != null) logger$1.warn("`legalComments` option is deprecated, please use `comments.legal` instead.");
...
function bindingifyCodeSplitting(codeSplitting, inlineDynamicImportsOption, advancedChunks, manualChunks) {
	...
	if (codeSplitting === false) {
		if (inlineDynamicImportsOption != null) logger$1.warn("`inlineDynamicImports` option is ignored because `codeSplitting: false` is set.");
		...
		if (advancedChunks != null) logger$1.warn("`advancedChunks` option is ignored because `codeSplitting` is set to `false`.");
		return {
			inlineDynamicImports: true,
			advancedChunks: void 0
		};
	} else if (codeSplitting === true) {
		if (inlineDynamicImportsOption != null) logger$1.warn("`inlineDynamicImports` option is ignored because `codeSplitting: true` is set.");
	} else if (codeSplitting == null) {
		if (inlineDynamicImportsOption != null) {
			logger$1.warn("`inlineDynamicImports` option is deprecated, please use `codeSplitting: false` instead.");
			inlineDynamicImports = inlineDynamicImportsOption;
		}
	} else {
		effectiveChunksOption = codeSplitting;
		if (inlineDynamicImportsOption != null) logger$1.warn("`inlineDynamicImports` option is ignored because the `codeSplitting` option is specified.");
	}
	...
	if (effectiveChunksOption == null) {
		if (advancedChunks != null) {
			logger$1.warn("`advancedChunks` option is deprecated, please use `codeSplitting` instead.");
			effectiveChunksOption = advancedChunks;
		}
	} else if (advancedChunks != null) logger$1.warn("`advancedChunks` option is ignored because the `codeSplitting` option is specified.");
	if (manualChunks != null && effectiveChunksOption != null) logger$1.warn("`manualChunks` option is ignored because the `codeSplitting` option is specified.");
...
//#endregion
//#region node_modules/rolldown/dist/shared/watch-Bak_bDNe.mjs
/**
...
var SignalExit = class extends SignalExitBase {
	/* c8 ignore start */
	#hupSig = process$1$1.platform === "win32" ? "SIGINT" : "SIGHUP";
	/* c8 ignore stop */
	#emitter = new Emitter();
	#process;
	#originalProcessEmit;
...
const process$1$1 = globalThis.process;
const { onExit: onExit$1, load, unload } = signalExitWrap(processOk(process$1$1) ? new SignalExit(process$1$1) : new SignalExitFallback());
...
require_binding();
//#endregion
//#region node_modules/rolldown/dist/shared/rolldown-hwU2j9UL.mjs
/**
...
const logger = {
	info: (message, ...args) => console.log(`${colors.blue("[INFO]")} ${colors.gray(message)}`, ...args),
	warn: (message, ...args) => console.log(`${colors.yellow("[WARN]")} ${colors.yellow(message)}`, ...args),
	error: (message, ...args) => console.log(`${colors.red("[ERROR]")} ${colors.red(message)}`, ...args),
	success: (message, ...args) => console.log(`${colors.green("[SUCCESS]")} ${colors.green(message)}`, ...args)
};
...
var Builder = class {
	...
	async build() {
		...
		try {
			...
			for (const output of outputOptions) await build.write(output);
			await build.close();
	...
	normalizeOutput(output) {
		...
		return (Array.isArray(output) ? output : [output]).map((o) => ({
			dir: o.dir || "dist",
			file: o.file,
			format: o.format || "esm",
...
async function loadConfig(userConfigPath) {
	...
	try {
		...
		return {
			...
			...typeof userConfig === "function" ? userConfig({ mode: process.env.NODE_ENV || "production" }) : userConfig
		};
	} catch (error) {
...
function parseArgs(args) {
	...
	for (let i = 0; i < args.length; i++) {
		const arg = args[i];
		if (arg === "-c" || arg === "--config") result.options.config = args[++i];
		else if (arg === "-p" || arg === "--port") result.options.port = args[++i];
		else if (arg === "-V" || arg === "--version") result.command = "version";
...
async function main() {
	...
	if (command === "check") try {
		await loadConfig(parsed.options.config);
		logger.info("Starting type checking...");
		if (await new TypeScriptCompiler().checkTypes()) {
	...
	else if (command === "dev") try {
		await new Builder(await loadConfig(parsed.options.config)).watch();
		logger.success(`Development server running at http://localhost:${parsed.options.port || 3e3}`);
	} catch (error) {
		logger.error("Error starting development server:", error);
		process.exit(1);
	}
	else if (command === "compile") try {
	...
	else if (command === "build") try {
		const config = await loadConfig(parsed.options.config);
		logger.info("Starting build process...");
		if (await new Builder(config).build()) {
			logger.success("Build completed successfully");
			process.exit(0);
		} else {
